...
class PursuitEnv(Node):
    def __init__(self, cfg, perception):
        ...
        self.create_subscription(
            ...
            self._chaser_odom_cb, MAVROS_QOS)
        self.create_subscription(
            Odometry, '/evader/odom',
            self._evader_odom_cb, 10)
        self.create_subscription(
            Image, '/iris/camera/image_raw',
            self._image_cb, 10)
    ...
    def get_state(self):
        ...
        chaser_speed = np.linalg.norm(self.chaser_vel)
        ...
        state = np.array([
            rel[0], rel[1], rel[2],
            self.chaser_vel[0], self.chaser_vel[1], self.chaser_vel[2],
            dist, heading_err, alt_err, float(self.vision_bit),
        ], dtype=np.float32)
        if self.bbox is not None:
            state = np.concatenate([state, np.array(self.bbox, dtype=np.float32)])
        return state
